...
class Order extends Base
{
    ...
    /*api:8acbb92e0777cdc79b6532ae4ad502cd*/
    /**
     * * 充值订单
     * @param  [array] $parame 接口参数
     * @return [array]         接口输出数据
     */
    private function listDataForAdmin($parame)
    {
        //主表数据库模型
        $dbModel                    = model($this->mainTable);
        ...
        //Rtype :`INNER`、`LEFT`、`RIGHT`、`FULL`，不区分大小写，默认为`INNER`。
        $RelationTab                = [];
        $RelationTab['user_center'] = array('Ralias'=>'uc','Ron'=>'uc.id=main.uid','Rtype'=>'LEFT','Rfield'=>array('username'));
        ...
        if (!empty($data)) {
            ...
            //自行定义格式化数据输出
            $pay_type    = ['1'=>'支付宝','2'=>'微信','3'=>'支付宝转账','4'=>'微信扫码','5'=>' 银联快捷'];
            foreach($data as $k=>$v){
                $data[$k]['pay_type']      = isset($pay_type[$v['pay_type']]) ? $pay_type[$v['pay_type']] : '/';
                $data[$k]['nickname']      = !empty($v['nickname']) ? $v['nickname'] : $v['username'];
                $data[$k]['status_text']   = $v['status'] == 2 ? '已支付' : '未支付';
